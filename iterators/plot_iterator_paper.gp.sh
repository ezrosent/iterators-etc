...
#  do for [m=1:words(nDuration)] {
    ...
    set output outputDir."/paper_iterators.png" 
    ...
    do for [k=1:nPlotsY] { # 2 rows = duration
		...
		set title "dis = " . word(nWeightConfig,j) . ", range = " . rangeTitle . ", # updaters = " . word(nUpdaters,k) font ", 14"
	    set origin originX,originY
            originX = originX + deltaX;
	    #print "fline = ". fline . " lline = ".lline
	    plot for [i=1:nLines] "parse_iterator_output.txt" using 1:columns(i) every incI::fline title word(titles,i) with linespoints linewidth 2 pointsize 1
