# plots for two distributions, one keyrange and different number of updaters
################
#	[0, 16384] 
#(25,25,50)   (50,50,0)
#	 |
...
#TODO: automate the variable initialization
...
nRange = "(16384,8192)" #"`head -5 config.txt | tail -1`"
...
#  do for [m=1:words(nDuration)] {
    ...
    set output outputDir."/paper_updaters.png" 
    ...
    do for [k=1:nPlotsY] { # 2 rows = duration
		...
		set title "dis = " . word(nWeightConfig,j) . ", range = " . rangeTitle . ", # iterators = " .word(nIterators,k) font ", 14"
	    set origin originX,originY
            originX = originX + deltaX;
	    print "fline = ". fline . " lline = ".lline
	    plot for [i=1:nLines] "parse_updater_output.txt" using 2:columns(i) every incU::fline::lline with linespoints linewidth 2 pointsize 1
