...
nLines = 1 # should be 3 when including LL
...
do for [l=1:nMultiplots] {
    ...
    titles = "Linked-list unbalanced-BST"
    ...
    do for [k=1:nPlotsY] { # 2 rows = duration
	    ...
	    plot for [i=1:nLines] "output.txt" using 2:8 every increment::firstLine::lastLine title word(titles,i) with linespoints linewidth 2 pointsize 1
