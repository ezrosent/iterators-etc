...
nIterators = "1 3 5 7" #"`head -1 config.txt`"
...
# there should be as many pairs as the number of iterators.
fline = flineI
lline = flineI + incI - 1
...
#  do for [m=1:words(nDuration)] {
    ...
    set output outputDir. "/paper_updaters_com.png" 
    ...
    do for [k=1:nPlotsY] { # 2 rows = duration
	    ...
	    plot for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline::lline with linespoints linewidth 2 pointsize 1, \
	    	for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline1::lline1 with linespoints linewidth 2 pointsize 1, \
	    	for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline2::lline2 with linespoints linewidth 2 pointsize 1, \
	    	for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline3::lline3 with linespoints linewidth 2 pointsize 1
	    #	for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline4::lline4 with linespoints linewidth 2 pointsize 1, \
	    #	for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline5::lline5 with linespoints linewidth 2 pointsize 1, \
	    #	for [i=1:nLines] "parse_updater_com_output.txt" using 2:columns(i) every incU::fline6::lline6 with linespoints linewidth 2 pointsize 1 
